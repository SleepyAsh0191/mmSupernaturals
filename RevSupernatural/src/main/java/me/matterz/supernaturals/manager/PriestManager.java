...
public class PriestManager extends HumanManager {
					...
					if (Math.abs(locZ - SNConfigHandler.priestChurchLocationZ) <= 10) {
						if (snplayer.isPriest()) {
							...
							if (snplayer.isSuper()) {
								...
								EntityDamageEvent event = new EntityDamageEvent(player, DamageCause.BLOCK_EXPLOSION,
										20);
	...
	public boolean exorcise(Player player, Player victim) {
		...
		if (snplayer.getPower() > SNConfigHandler.priestPowerExorcise) {
			if (snvictim.isSuper()) {
				SuperNManager.alterPower(snplayer, -SNConfigHandler.priestPowerExorcise,
						"Exorcised " + victim.getName());
				SuperNManager.sendMessage(snvictim,
	...
	public boolean drainPower(Player player, Player victim) {
		...
		if (snplayer.getPower() > SNConfigHandler.priestPowerDrain) {
			if (snvictim.isSuper()) {
				...
				SuperNManager.alterPower(snplayer, -SNConfigHandler.priestPowerDrain,
		...
		if (priest.getPower() > SNConfigHandler.priestPowerGuardianAngel) {
			if (!snvictim.isSuper()) {
				if (SupernaturalsPlugin.instance.getDataHandler().hasAngel(priest)) {
					SuperNManager.sendMessage(priest, "Removed Guardian Angel from " + ChatColor.WHITE
				...
				SuperNManager.alterPower(priest, -SNConfigHandler.priestPowerGuardianAngel,
