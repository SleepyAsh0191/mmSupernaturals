...
import me.matterz.supernaturals.util.PlayerUtil;

public class DemonManager extends ClassManager {

	public static final Biome[] SET_VALUES = new Biome[] { Biome.ICE_FLATS, Biome.MUTATED_ICE_FLATS,
			Biome.ICE_MOUNTAINS, Biome.TAIGA, Biome.TAIGA_COLD, Biome.TAIGA_COLD_HILLS, Biome.FROZEN_OCEAN,
			Biome.FROZEN_RIVER, Biome.COLD_BEACH };

	public static final Set<Biome> FROZEN_BIOMES = new HashSet<Biome>(Arrays.asList(SET_VALUES));
	...
	public double victimEvent(EntityDamageEvent event, double damage) {
		...
		if (event.getCause().equals(DamageCause.FIRE) || event.getCause().equals(DamageCause.BLOCK_EXPLOSION)
			...
			if (!demons.contains(dPlayer)) {
				...
				SupernaturalsPlugin.instance.getServer().getScheduler()
						.scheduleSyncDelayedTask(SupernaturalsPlugin.instance, new Runnable() {
							@Override
							public void run() {
		...
		if (item != null)
			if (SNConfigHandler.demonWeapons.contains(item.getType())) {
				if (SNConfigHandler.debugMode) {
					SupernaturalsPlugin
							.log(pDamager.getName() + " was not allowed to use " + item.getType().toString());
				}
	...
	public void deathEvent(Player player) {
		...
		if (e.getCause().equals(DamageCause.DROWNING)) {
			...
			if (snplayer.isDemon()) {
				...
				if (FROZEN_BIOMES.contains(pBiome)) {
					if (SNConfigHandler.debugMode) {
	...
	public boolean convert(Player player, Player target) {
		...
		if (target.getItemInHand().getType().equals(Material.NETHERRACK)) {
			...
			SuperNManager.sendMessage(snvictim,
					ChatColor.RED + "You have been converted to a demon by " + player.getName());
			return true;
		}
		return false;
	}
	...
	public boolean snare(Player player, Player target) {
		...
		if (target == null) {
			block = PlayerUtil.getTargetBlock(player, 20);
		} else {
